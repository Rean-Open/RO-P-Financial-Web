...
import ExpenseModal from "../../containers/modal/expenseModal";
...
const Expense = (props) => {
  ...
  const fetchUserFormList = () => {
    setLoading(true);
    API.get(`${ACTIVITY_LIST_INCOME}?userId=${user.id}&activityTypeId=2`)
      .then((res) => {
        setLoading(false);
        if (res.status === 200) {
  ...
  return (
    ...
    >
      <h1 className="size20700 mt-5">All Expense</h1>
      <div className="mt-3">
